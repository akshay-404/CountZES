...
def get_arguments():
    ...
    parser.add_argument('--outdir', type=str, default='countzes_persenseD')
...
def main():
    ...
    images_path = "./data/persenseD/Images/"
    masks_path = args.data + '/persenseD/Images/'
...
def countzes(args, obj_name, images_path, masks_path, output_path, sam, grounding_dino_model, counter_model, infer_time, clip_model, clip_processor, count_item, mae, mse, anyup):
    ...
    for test_idx in tqdm(range(loop_over//2)):
        ...
        img_file = test_idx + '.jpg'
        test_image_path = test_images_path + '/' + test_idx + '.jpg'
        anno_path = test_images_path  + '/' + 'points_' + obj_name + '.json'
        filename = img_file
        ...
        for entry in data:
            if entry['img'] == img_file:
                gt_count = entry['total_points']
                break     
