    ...
    def crop_roi_feat(self,feat, boxes, out_stride, exemplar_pool_size):
        ...
        for jdx_batch in range(0, boxes.shape[0]):
            for idx_box in range(0, boxes.shape[1]):
                ...
                if feat_box.shape[1] == 0 or feat_box.shape[2] == 0:
                    # Instead of continue, create a dummy tensor so list length remains consistent
                    feat_box = torch.zeros((feat_box.shape[0], 1, 1), device=feat_box.device)
