...
def get_arguments():
    ...
    parser.add_argument('--outdir', type=str, default='countzes_carpk')
...
def countzes(args, obj_name, images_path, masks_path, output_path, sam, grounding_dino_model, counter_model, infer_time, clip_model, clip_processor, anyup):
    ...
    json_file_path = './data/CARPK/test.json'
    data_list = []
    count_item = 0
    mae = 0
    mse = 0
    ...
    with open(json_file_path, 'r') as f:
        for line in f:
            try:
                ...
                continue
    preprocessed_json_file_path = './data/CARPK/preprocessed_test.json'
    with open(preprocessed_json_file_path, 'w') as f:
        json.dump(data_list, f, indent=4)
    print(f"Preprocessed JSON data saved to {preprocessed_json_file_path}")
    json_file_path = './data/CARPK/preprocessed_test.json'
    image_folder_path = './data/CARPK/Images/'
    try:
        with open(json_file_path, 'r') as f:
            data = json.load(f)
    except json.JSONDecodeError as e:
    ...
    for entry in data:      
        ...
        density_name = entry['density']
        gt_path = "./data/CARPK/gt_density_map/"
        gt_img_path = os.path.join(gt_path, density_name)
        gt_density = np.load(gt_img_path)
        ...
        if args.visualize == True:
            set_path = './outputs/' + 'countzes_carpk'
            if not os.path.exists(set_path):
                os.mkdir('./outputs/countzes_carpk')
