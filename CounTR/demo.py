...
def load_image(test_image_path, ori_boxes):
    ...
    # Process ori_boxes directly (list of [x1, y1, x2, y2])
    bboxes = []
    rects = []
    boxes = []
    ...
    # for bbox in bboxes:
    #     # Scale coordinates
    #     x1_scaled = int(x1 * scale_factor_W)
    #     y1_scaled = int(y1 * scale_factor_H)
    #     x2_scaled = int(x2 * scale_factor_W)
    #     y2_scaled = int(y2 * scale_factor_H)

    #     rects.append([y1_scaled, x1_scaled, y2_scaled, x2_scaled])

    #     # Crop and resize exemplar patch
    #     crop = image[:, y1_scaled:y2_scaled + 1, x1_scaled:x2_scaled + 1]
    #     crop = transforms.Resize((64, 64))(crop)
    #     boxes.append(crop.numpy())

    # boxes = np.array(boxes)
    # boxes = torch.Tensor(boxes)

    # if boxes.shape[1] == 4:  # drop alpha channel if present
    #     boxes = boxes[:, :3, :, :]
...
def _resize_density_sum_preserving(density_hw: np.ndarray, out_w: int, out_h: int) -> np.ndarray:
    """
    Resize a scalar density map [H,W] to [out_h, out_w] while preserving the integral (sum).
    """
    H, W = density_hw.shape
    if H == out_h and W == out_w:
        return density_hw
    resized = cv2.resize(density_hw, (out_w, out_h), interpolation=cv2.INTER_LINEAR)
    # multiply by area ratio so global sum is preserved
    scale = (W * H) / float(out_w * out_h)
    return resized * scale

def run_one_image(samples, boxes, pos, model, W, H, test_idx, new_W, new_H, device="cuda"):
    ...
    if s_cnt >= 1:
        ...
        with measure_time() as et:
            # accumulate a full-resolution density_map for each region, then sum counts
            for r_image in r_images:
                ...
                with torch.no_grad():
                    while start + 383 < w:
                        ...
                        d1 = b1(output[:, 0:prev - start + 1])               # left overlap
                        b2 = nn.ZeroPad2d(padding=(prev + 1, w - start - 384, 0, 0))
                        d2 = b2(output[:, prev - start + 1:384])              # right new
                ...
                # Convert to raw density-per-pixel (divide by 60 as used for pred_cnt)
                density_map_raw = (density_map / 60.0).detach().cpu().numpy().astype(np.float32)  # [h, w]
                pred_cnt += float(density_map_raw.sum())  # global count contributed by this region
                r_densities.append(density_map)           # keep original torch map if needed

            # Prepare outputs for visualization from the last 'density_map' (or fuse if you prefer)
            density_vis = density_map.detach().cpu().numpy().astype(np.float32)
            if new_W < 384:
                crop_dim = (384 - new_W) // 2
                density_vis = density_vis[0:384, crop_dim:crop_dim + new_W]
            # ---- RAW (sum-preserving) at image size ----
            density_raw = _resize_density_sum_preserving(density_vis / 60.0, W, H)  # single-channel float [H,W]
            # ---- VIS (normalized 0..1) at image size ----
            norm = (density_vis - density_vis.min()) / (density_vis.max() - density_vis.min() + 1e-8)
            density_pred = cv2.resize(norm, (W, H))
            density_pred = np.stack((density_pred,) * 3, axis=-1)  # 3ch for overlay
        ...
        with measure_time() as et:
            pass  # keep et defined for return signature

        # --- counts ---
        pred_cnt = float((density_map / 60.0).sum().item())
        ...
        # RAW (sum-preserving) at image size
        density_raw = _resize_density_sum_preserving(density_vis / 60.0, W, H)   # [H,W] float32

        # VIS (normalized) at image size for overlay
        norm = (density_vis - density_vis.min()) / (density_vis.max() - density_vis.min() + 1e-8)
        density_pred = cv2.resize(norm, (W, H))
        density_pred = np.stack((density_pred,) * 3, axis=-1)

    # e_cnt scaling as in your original (if you rely on it)
    e_cnt = 0.0
    for rect in pos:
        y0, x0, y1, x1 = rect
        y0 = max(0, min(h - 1, y0)); y1 = max(0, min(h - 1, y1))
        x0 = max(0, min(w - 1, x0)); x1 = max(0, min(w - 1, x1))
        e_cnt += float((density_map[y0:y1 + 1, x0:x1 + 1] / 60.0).sum().item())
    e_cnt /= 3.0
    if e_cnt > 1.8:
        pred_cnt /= e_cnt

    # Return: global count, elapsed time ctx, VIS heatmap (3ch), RAW sum-preserving (1ch)
    return pred_cnt, et, density_pred, density_raw
